...
def calculate_local_features(data):
    ...
    try:
        ...
        # Debug: Log raw data
        if data.get('compound') in ['Ag1Cl1O4', 'Ag1Na1O1', 'Ag1K1O1', 'Ag1O1Rb1', 'Ag1O1', 'Ag4O12Sb4', 'Ag4Mo2O8', 'Ni4O8Si2', 'Eu4O14Sn4', 'Ni1O1', 'Ag26O12Os2']:
            print(f"Debug: Raw data for {data.get('compound')}:")
            print(f"  positions_fractional: {data.get('positions_fractional')}")
        ...
        expected_a = 5.0 if 'O4' in data.get('compound') else 4.8  # Larger for AgClO4, smaller for AgMO
        if data.get('sg', '').startswith(('Fm-3c', 'F-43m')):
            ...
            if abs(data.get('volume_cell', 0) - volume_expected) / volume_expected > 0.1:
        ...
        max_cutoff = 4.0  # Maximum cutoff for any bond
        ...
        for idx, site in enumerate(structure):
            ...
            # Debug: Warn on zero coordination
            if coordination_number == 0 and data.get('compound') in ['Ag1Cl1O4', 'Ag1Na1O1', 'Ag1K1O1', 'Ag1O1Rb1', 'Ag1O1', 'Ag4O12Sb4', 'Ag4Mo2O8', 'Ni4O8Si2', 'Eu4O14Sn4', 'Ni1O1', 'Ag26O12Os2']:
                print(f"Warning: Zero coordination for {data['compound']}, {site.species_string}. Check cutoff or geometry.")
                print(f"  Lattice: {lattice}")
                print(f"  All neighbors: {[(n.species_string, n[1]) for n in site_neighbors]}")
                print(f"  Expected bond range: {bond_ranges.get(f'{site.species_string}-O', (1.4, max_cutoff))}")

            # Debug: Print for problematic structures
            if data.get('compound') in ['Ag1Cl1O4', 'Ag1Na1O1', 'Ag1K1O1', 'Ag1O1Rb1', 'Ag1O1', 'Ag4O12Sb4', 'Ag4Mo2O8', 'Ni4O8Si2', 'Eu4O14Sn4', 'Ni1O1', 'Ag26O12Os2']:
                print(f"Debug: {data['compound']}, {site.species_string}")
                print(f"  Lattice: {lattice}")
                print(f"  Neighbors: {[(n.species_string, n[1]) for n in relevant_neighbors]}")
                print(f"  Bond lengths: {bond_lengths}")
                print(f"  Average bond length: {average_bond_length}")
...
def get_structure_info(data):
    """
    Parses the AFLOW JSON file to extract key structural information, handling list or string formats.
    """
    try:
        # Handle species
        species = data.get("species", [])
...
def fetch_aflow_data_batch(aflow_uids, structures, aflow_url="http://aflowlib.duke.edu"):
    """
    Fetch data for multiple UIDs and structures from AFLOWLIB.
    """
    ...
    for uid in aflow_uids:
        for structure in structures:
            processed_count += 1
            fetch_aflow_data(uid, structure, aflow_url=aflow_url)
